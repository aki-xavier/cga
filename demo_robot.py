...
两个坐标约定转换都在场景构建层, 数据文件保持 URDF 语义 (Z-up):
  1. 根级: URDF Z-up → 引擎 Y-up (RobotDemo.WORLD_UP = Rot(X, -π/2))。
...
class RobotDemo:
    ...
    def build_scene(robot: Robot, q: list[float]) -> Scene:
        ...
        for link in robot.links:
            m = RobotDemo.WORLD_UP.gp(world[link.name])
            for g in link.geometry:
                ...
                scene.add(Mesh(RobotDemo.to_geometry(g), mat, motor=m.gp(g.origin)))
        return scene
    ...
    def main() -> None:
        ...
        model = Path(sys.argv[2]) if len(sys.argv) > 2 else RobotDemo.MODEL
        robot = RobotLoader.load(model)
        # 默认: Z1 舒展姿态 (肩偏航 0.3 + 上臂前倾 120° + 前臂前伸)
        default_q = [0.3, 2.0944, -2.0944, 0.4, 0.3, 0.5]
        q = [float(v) for v in sys.argv[3:]] or default_q
        scene = RobotDemo.build_scene(robot, q)
        camera = PerspectiveCamera(
            fov=50,
            aspect=4 / 3,
            position=(0.45, 0.22, 0.75),
            target=(0.25, 0.30, 0.08),
        )
        camera.look_at((0.25, 0.30, 0.08))
        img = Renderer(640, 480, aa=2).render(scene, camera)
        fname = (
            "robot_z1.png"
            if model == RobotDemo.MODEL
            else f"robot_{robot.name}.png"
        )
        p = out_dir / fname
        Image.frombytes(
            "RGBA", (img.shape[1], img.shape[0]), Renderer.frame_to_bytes(img)
        ).save(p)
        print(f"saved {p}")
