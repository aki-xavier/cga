...
class AdvantageDemo:
    ...
    def render(
        ...
        """
        W, H = AdvantageDemo.W, AdvantageDemo.H
        camera = PerspectiveCamera(
            fov=50, aspect=W / H, position=cam_pos, target=target
        )
        camera.look_at(target)
        img = Renderer(W, H, aa=2).render(scene, camera)
        return Renderer.frame_to_bytes(img)
    ...
    def save_panel(
        ...
        """渲染一帧 + 底部说明文字 → 独立 PNG。"""
        W, H = AdvantageDemo.W, AdvantageDemo.H
        frame = AdvantageDemo.render(scene, cam_pos, target)
        canvas = Image.new("RGB", (W, H + 26), (16, 18, 22))
        canvas.paste(Image.frombytes("RGBA", (W, H), frame).convert("RGB"), (0, 0))
        draw = ImageDraw.Draw(canvas)
        font = AdvantageDemo.label_font()
        draw.text((8, H + 6), label, fill=(232, 232, 232), font=font)
        p = out_dir / name
        canvas.save(p)
        print(f"saved {p}")
        return p
    ...
    def main() -> None:
        ...
        AdvantageDemo.save_panel(
            "advantage_a.png",
            AdvantageDemo.panel_a_scene(),
            (0, 0.15, 3.2),
            (0, 0.15, 0),
