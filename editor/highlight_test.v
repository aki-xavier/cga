...
fn span_classes(text string) []string {
	mut out := []string{}
...
fn test_highlight_keywords_and_types() {
	...
	assert classes.contains('type') // sphere
...
fn test_highlight_classify_word() {
	assert classify_word('module') or { '' } == 'keyword'
	assert classify_word('sphere') or { '' } == 'type'
	assert classify_word('sqrt') or { '' } == 'function'
	assert classify_word('pi') or { '' } == 'constant'
...
fn test_highlight_color_map() {
	...
	assert highlight_color('unknown') == 0xdfe2ea
}
