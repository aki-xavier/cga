...
def fk(robot: Robot, q: dict[str, float]) -> dict[str, Motor]:
    ...
    """
    missing = [j.name for j in robot.joints if j.type in MOVABLE and j.name not in q]
    if missing:
        raise RobotError(f"fk 缺少关节角度: {missing}")
    world: dict[str, Motor] = {robot.base: Motor.identity()}
...
def fk_list(robot: Robot, q: list[float]) -> dict[str, Motor]:
    """fk 的便捷形式: q 按 robot.joints 顺序 (只含 movable 关节)。"""
    movable = [j for j in robot.joints if j.type in MOVABLE]
    if len(q) != len(movable):
        raise RobotError(
            f"fk_list 需要 {len(movable)} 个值 (movable 关节数), got {len(q)}"
        )
    return fk(robot, {j.name: v for j, v in zip(movable, q, strict=True)})
