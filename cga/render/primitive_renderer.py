"""图元渲染器: 把 CGA 图元场景 (SceneModel / 任意 blade 列表) 渲染回 2D 图像。
...
from cga.algebra import Cylinder, Plane, Sphere
...
class PrimitiveRenderer:
    ...
    @staticmethod
    def plane_params(b: Multivector) -> tuple[mx.array, float]:
    ...
    @staticmethod
    def sphere_params(b: Multivector) -> tuple[mx.array, float]:
    ...
    def render_scene(
        ...
        for p, b in zip(prims, cams, strict=True):
            ...
            if p.kind == "plane":
                ...
                q_c, n_c, r_c = PrimitiveRenderer.cylinder_params(
                    cast(Cylinder, b)  # kind 已保证
                )
                ...
                c, r = PrimitiveRenderer.sphere_params(b)
            ...
            col = mx.array(
                PrimitiveRenderer.PALETTE[p.region % len(PrimitiveRenderer.PALETTE)],
                dtype=mx.float32,
            )
    ...
    def selftest_cylinder() -> None:
        ...
        out = PrimitiveRenderer.render_scene(prims, K, (H, W))
        ...
        out2 = PrimitiveRenderer.render_scene(prims, K, (H, W), regions=regions2)
    ...
    @staticmethod
    def selftest() -> None:
        """合成场景自检 (掩码 + 全量 + motor + 裁剪 + alpha)。"""
        K = (100.0, 100.0, 64.0, 48.0)
        H, W = 96, 128
        render = PrimitiveRenderer.render_scene

        # 场景: 地平面 z=2 (region 1) + 球心 (0,0,3) r=0.5 (region 2)
        plane = Plane((0.0, 0.0, 1.0), 2.0)
        sphere = Sphere((0.0, 0.0, 3.0), 0.5)
        prims = [
            RenderPrimitive("plane", plane, 1),
            RenderPrimitive("sphere", sphere, 2),
        ]
        ...
        out = render(prims, K, (H, W), regions=regions)
        ...
        out4 = render([RenderPrimitive("plane", plane, 1)], K, (H, W), regions=half)
        ...
        PrimitiveRenderer.selftest_cylinder()

        # 半透明: 球 (z=2, r=0.5) 在墙 (z=4) 前, alpha 扫描
        wall = RenderPrimitive("plane", plane_far, 1)
        ball = RenderPrimitive("sphere", sphere_near, 2)
        rgb_wall = render([wall], K, (H, W)).rgb[48, 64]
        rgb_ball = render([ball, wall], K, (H, W)).rgb[48, 64]
