...
pub fn render_scene(prims []RenderPrimitive, fx f64, fy f64, cx f64, cy f64, h int, w int, regions ?mlx.Array, motor ?Multivector, near f64, far f64) RenderResult {
	...
	dirs := mlx.stack([mlx.s_div(mlx.s_sub(xx, cx), fx), mlx.s_div(mlx.s_sub(yy, cy), fy),
		mlx.ones([h, w], .float32)], -1)
	mut light := mlx.arr3(0.3, 0.6, 1.0)
	...
	for p in prims {
		...
		mut nrm := mlx.Array{}
		if p.kind == .plane {
			n := mlx.arr3v(b.euclidean_vector())
			d := b.einf_coeff()
			mut denom := dirs.multiply(n).sum_axis(-1, false)
			denom = mlx.where(mlx.s_gt(denom.abs(), 1e-8), denom, mlx.full_like(denom,
				mlx.f32_scalar(f32(math.inf(1))), .float32))
			t = mlx.fs(d).divide(denom)
			nrm = n.broadcast_to([h, w, 3])
		} else if p.kind == .sphere {
			c, r := sphere_from_dual(b)
			ca := mlx.arr3v(c)
			a := dirs.multiply(dirs).sum_axis(-1, false)
			bb := mlx.s_mul(dirs.multiply(ca).sum_axis(-1, false), -2.0)
			cc := mlx.s_sub(ca.multiply(ca).sum(), r * r)
			disc := bb.multiply(bb).subtract(mlx.s_mul(a.multiply(cc), 4.0))
			t = mlx.where(mlx.s_gt(disc, 0.0),
				bb.negative().subtract(mlx.s_max(disc, 0.0).sqrt()).divide(mlx.s_mul(a, 2.0)), mlx.full_like(a,
				mlx.f32_scalar(f32(math.inf(1))), .float32))
			hit := t.expand_dims(2).multiply(dirs).subtract(ca)
			nrm = hit.divide(mlx.s_max(hit.multiply(hit).sum_axis(-1, true).sqrt(), 1e-8))
		} else {
			cyl := p.cylinder_data or { panic('cylinder render needs cylinder_data') }
			q := mlx.arr3v(cyl.axis_point)
			n := mlx.arr3v(cyl.axis_dir)
			r := cyl.radius
			...
			cq := mlx.s_sub(q.multiply(q).sum().subtract(qn.multiply(qn)), r * r)
			disc := bq.multiply(bq).subtract(aq.multiply(cq))
			t = mlx.where(mlx.s_gt(aq, 1e-8).logical_and(mlx.s_gt(disc, 0.0)),
				bq.negative().subtract(mlx.s_max(disc, 0.0).sqrt()).divide(aq), mlx.full_like(aq,
				mlx.f32_scalar(f32(math.inf(1))), .float32))
			hit := t.expand_dims(2).multiply(dirs).subtract(q)
			rad := hit.subtract(hit.multiply(n).sum_axis(-1, true).multiply(n))
			nrm = rad.divide(mlx.s_max(rad.multiply(rad).sum_axis(-1, true).sqrt(), 1e-8))
		}
		t = mlx.where(mlx.s_gt(t, near).logical_and(mlx.s_lt(t, far)), t, mlx.full_like(t,
			mlx.f32_scalar(f32(math.inf(1))), .float32))
		t = mlx.where(sel, t, mlx.full_like(t, mlx.f32_scalar(f32(math.inf(1))), .float32))
		best = best.minimum(t)
		base := mlx.arr3v(palette_rgb[p.region % 12])
		sh := mlx.s_max(nrm.multiply(light).sum_axis(-1, false), 0.0)
		rgb_p :=
			base.expand_dims(0).expand_dims(0).multiply(mlx.s_add(mlx.s_mul(sh, 0.65), 0.35).expand_dims(2))
		valid1 := mlx.s_lt(t, math.inf(1))
		hits_t << t
		hits_rgb << mlx.where(valid1.expand_dims(2), rgb_p, mlx.zeros_like(rgb_p))
		hits_a << mlx.where(valid1, mlx.fs(p.alpha), mlx.fs(0.0))
	}
	...
	depth := mlx.where(mlx.s_lt(best, far), best, mlx.fs(0.0))
	return RenderResult{
		depth: depth
		rgb:   acc
	}
