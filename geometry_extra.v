...
fn dk_roots(c3 mlx.Array, c2 mlx.Array, c1 mlx.Array, c0 mlx.Array) mlx.Array {
	rad := mlx.s_add(mlx.stack([c3.abs(), c2.abs(), c1.abs(), c0.abs()], -1).max_axis(-1, true), 1.0)
	...
	re := z.real()
	im := z.imag()
	real_ok := im.abs().less_equal(mlx.s_mul(mlx.s_max(re.abs(), 1.0), 1e-3))
...
fn cone_local_interval(r f64, h f64, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array, mlx.Array, mlx.Array, mlx.Array) {
	k := r / h
	k2 := 1.0 + k * k
	wz := mlx.s_sub(col(o, 2), h / 2.0)
	dz := col(d, 2)
	wd := o.multiply(d).sum_axis(-1, false).subtract(mlx.s_mul(dz, h / 2.0))
	ww := mlx.s_add(o.multiply(o).sum_axis(-1, false).subtract(mlx.s_mul(col(o, 2), h)), h * h / 4.0)
	a := mlx.fs(1.0).subtract(mlx.s_mul(dz.multiply(dz), k2))
	b := mlx.s_mul(wd.subtract(mlx.s_mul(wz.multiply(dz), k2)), 2.0)
	c := ww.subtract(mlx.s_mul(wz.multiply(wz), k2))
	mut side_ok := mlx.s_gt(a.abs(), 1e-12)
	a_s := mlx.where(side_ok, a, mlx.full_like(a, mlx.f32_scalar(f32(1e-12)), .float32))
	disc := b.multiply(b).subtract(mlx.s_mul(a_s.multiply(c), 4.0))
	side_ok = side_ok.logical_and(mlx.s_gt(disc, 1e-12))
	sq := mlx.s_max(disc, 0.0).sqrt()
	r_lo := b.negative().subtract(sq).divide(mlx.s_mul(a_s, 2.0))
	r_hi := b.negative().add(sq).divide(mlx.s_mul(a_s, 2.0))
	st0 := r_lo.minimum(r_hi)
	st1 := r_lo.maximum(r_hi)
	safe_dz := mlx.where(mlx.s_gt(dz.abs(), 1e-9), dz, mlx.full_like(dz, mlx.f32_scalar(f32(1e-9)),
		.float32))
	t_top := wz.negative().divide(safe_dz)
	t_bot := mlx.s_add(wz, h).negative().divide(safe_dz)
	at0 := t_top.minimum(t_bot)
	at1 := t_top.maximum(t_bot)
	pos_a := mlx.s_gt(a, 1e-12)
	...
	ez := mlx.arr3(0.0, 0.0, 1.0).expand_dims(0)
...
fn cone_side_n(h f64, k2 f64, o mlx.Array, d mlx.Array, t mlx.Array) mlx.Array {
	...
	s := mlx.s_sub(col(p, 2), h / 2.0)
	g := mlx.stack([col(p, 0), col(p, 1), mlx.s_mul(s, 1.0 - k2)], -1)
	norm := g.multiply(g).sum_axis(-1, true).sqrt()
	return g.divide(mlx.where(mlx.s_gt(norm, 1e-12), norm, mlx.ones_like(norm)))
}
...
fn cone_local_intersect(r f64, h f64, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array, mlx.Array) {
	...
	hit_enter := valid.logical_and(mlx.s_gt(enter, 1e-6))
	hit_exit := valid.logical_and(hit_enter.logical_not()).logical_and(mlx.s_gt(exit_, 1e-6))
...
pub fn cone_shadow(p ConeParams, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array) {
	...
	hit_enter := valid.logical_and(mlx.s_gt(enter, 1e-6))
	t := mlx.where(hit_enter, enter, exit_)
	mask := hit_enter.logical_or(valid.logical_and(mlx.s_gt(exit_, 1e-6)))
	return t.divide(col(lam, 0)), mask
}

pub fn cone_uv(p ConeParams, pos mlx.Array, n mlx.Array) mlx.Array {
	p_l := vecmat(pos, mat3_transpose(p.a_inv3)).add(mlx.arr3v(p.t_inv))
	u := mlx.s_add(mlx.s_div(col(p_l, 1).arctan2(col(p_l, 0)), 2.0 * math.pi), 0.5)
	v := mlx.s_div(mlx.s_rsub(col(p_l, 2), p.h / 2.0), p.h)
...
pub fn ellipsoid_intersect(p EllipsoidParams, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array, mlx.Array) {
	...
	t_l := mlx.where(valid.logical_and(mlx.s_gt(t1, 1e-6)), t1, t2)
	mask := valid.logical_and(mlx.s_gt(t_l, 1e-6))
...
pub fn ellipsoid_shadow(p EllipsoidParams, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array) {
	...
	t2 := mlx.s_div(b.negative().add(sq), 2.0)
	t_l := mlx.where(valid.logical_and(mlx.s_gt(t1, 1e-6)), t1, t2)
	return t_l.divide(col(lam, 0)), valid.logical_and(mlx.s_gt(t_l, 1e-6))
}

pub fn ellipsoid_uv(p EllipsoidParams, pos mlx.Array, n mlx.Array) mlx.Array {
	p_l := vecmat(pos, mat3_transpose(p.a_inv3)).add(mlx.arr3v(p.t_inv))
	u := mlx.s_add(mlx.s_div(col(p_l, 1).arctan2(col(p_l, 0)), 2.0 * math.pi), 0.5)
	v := mlx.s_div(mlx.s_clip(col(p_l, 2), -1.0, 1.0).arccos(), math.pi)
	return mlx.stack([u, v], -1)
}
...
fn torus_local_crossings(major f64, minor f64, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array, mlx.Array) {
	...
	g := mlx.s_add(oo, r2 - minor * minor)
	c3 := mlx.s_mul(od, 4.0)
	c2 := mlx.s_mul(g, 2.0).add(mlx.s_mul(od.multiply(od), 4.0)).subtract(mlx.s_mul(col(d, 0).multiply(col(d, 0)).add(col(d, 1).multiply(col(d, 1))),
		4.0 * r2))
	c1 := mlx.s_mul(od.multiply(g), 4.0).subtract(mlx.s_mul(col(o, 0).multiply(col(d, 0)).add(col(o, 1).multiply(col(d, 1))),
		8.0 * r2))
	c0 := g.multiply(g).subtract(mlx.s_mul(col(o, 0).multiply(col(o, 0)).add(col(o, 1).multiply(col(o, 1))),
		4.0 * r2))
...
fn torus_local_contains(major f64, minor f64, p mlx.Array) mlx.Array {
	r2 := major * major
	f := mlx.s_add(p.multiply(p).sum_axis(-1, false), r2 - minor * minor).square().subtract(mlx.s_mul(col(p, 0).multiply(col(p, 0)).add(col(p, 1).multiply(col(p, 1))),
		4.0 * r2))
	return mlx.s_lt(f, 0.0)
}
...
pub fn torus_intersect(p TorusParams, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array, mlx.Array) {
	...
	pos := valid.logical_and(mlx.s_gt(ts, 1e-6))
...
pub fn torus_uv(p TorusParams, pos mlx.Array, n mlx.Array) mlx.Array {
	...
	u := mlx.s_add(mlx.s_div(col(p_l, 1).arctan2(col(p_l, 0)), 2.0 * math.pi), 0.5)
	v := mlx.s_add(mlx.s_div(col(p_l, 2).arctan2(mlx.s_sub(rho, p.major)), 2.0 * math.pi), 0.5)
	return mlx.stack([u, v], -1)
}
...
fn cyclide_local_crossings(a f64, b f64, dd f64, c f64, shift [3]f64, o mlx.Array, d mlx.Array) (mlx.Array, mlx.Array, mlx.Array) {
	ox := mlx.s_sub(col(o, 0), shift[0])
	oy := mlx.s_sub(col(o, 1), shift[1])
	oz := mlx.s_sub(col(o, 2), shift[2])
	dx := col(d, 0)
	dy := col(d, 1)
	dz := col(d, 2)
	...
	bb := b * b - dd * dd
	g := mlx.s_add(big_a, bb)
	p0 := mlx.s_sub(mlx.s_mul(ox, a), c * dd)
	p1 := mlx.s_mul(dx, a)
	c3 := mlx.s_mul(b1, 4.0)
	c2 := mlx.s_mul(g, 2.0).add(mlx.s_mul(b1.multiply(b1), 4.0)).subtract(mlx.s_mul(p1.multiply(p1), 4.0)).subtract(mlx.s_mul(dy.multiply(dy),
		4.0 * b * b))
	c1 := mlx.s_mul(b1.multiply(g), 4.0).subtract(mlx.s_mul(p0.multiply(p1), 8.0)).subtract(mlx.s_mul(oy.multiply(dy),
		8.0 * b * b))
	c0 := g.multiply(g).subtract(mlx.s_mul(p0.multiply(p0), 4.0)).subtract(mlx.s_mul(oy.multiply(oy),
		4.0 * b * b))
	ts := dk_roots(c3, c2, c1, c0)
	valid := ts.isfinite()
	safe_t := mlx.where(valid, ts, mlx.zeros_like(ts))
	p :=
		o.expand_dims(1).subtract(mlx.arr3v(shift).expand_dims(0).expand_dims(0)).add(safe_t.expand_dims(2).multiply(d.expand_dims(1)))
...
fn cyclide_normal(a f64, b f64, dd f64, c f64, p mlx.Array) mlx.Array {
	...
	p1 := p.take_axis(mlx.int_scalar(1), 2)
	p2 := p.take_axis(mlx.int_scalar(2), 2)
	grad := mlx.stack([mlx.s_mul(p0.multiply(g0), 4.0).subtract(mlx.s_mul(mlx.s_sub(mlx.s_mul(p0, a), c * dd),
		8.0 * a)),
		mlx.s_mul(p1.multiply(g0), 4.0).subtract(mlx.s_mul(p1, 8.0 * b * b)),
		mlx.s_mul(p2.multiply(g0), 4.0)], -1)
	norm := grad.multiply(grad).sum_axis(-1, true).sqrt()
	return grad.divide(mlx.where(mlx.s_gt(norm, 1e-12), norm, mlx.ones_like(norm)))
}

fn cyclide_local_contains(a f64, b f64, dd f64, c f64, shift [3]f64, p mlx.Array) mlx.Array {
	x := mlx.s_sub(col(p, 0), shift[0])
	y := mlx.s_sub(col(p, 1), shift[1])
	z := mlx.s_sub(col(p, 2), shift[2])
	bb := b * b - dd * dd
	rho := x.multiply(x).add(y.multiply(y)).add(z.multiply(z))
	f := mlx.s_add(rho, bb).square().subtract(mlx.s_mul(mlx.s_sub(mlx.s_mul(x, a), c * dd).square(), 4.0)).subtract(mlx.s_mul(y.multiply(y),
		4.0 * b * b))
	return mlx.s_lt(f, 0.0)
}
...
pub fn cyclide_uv(p CyclideParams, pos mlx.Array, n mlx.Array) mlx.Array {
	p_l := vecmat(pos, mat3_transpose(p.a_inv3)).add(mlx.arr3v(p.t_inv))
	x := mlx.s_sub(col(p_l, 0), p.shift[0])
	y := mlx.s_sub(col(p_l, 1), p.shift[1])
	z := mlx.s_sub(col(p_l, 2), p.shift[2])
	rho := x.multiply(x).add(y.multiply(y)).add(z.multiply(z))
	u := mlx.s_add(mlx.s_div(mlx.s_mul(y, 2.0 * p.b).arctan2(mlx.s_mul(mlx.s_sub(mlx.s_mul(x, p.a), p.c * p.d), 2.0)),
		2.0 * math.pi), 0.5)
	v :=
		mlx.s_add(mlx.s_div(mlx.s_mul(z, 2.0 * p.b).arctan2(mlx.s_rsub(rho, p.d * p.d + p.b * p.b)), 2.0 * math.pi), 0.5)
	return mlx.stack([u, v], -1)
}
